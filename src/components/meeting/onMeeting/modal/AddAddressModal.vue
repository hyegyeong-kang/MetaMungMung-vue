  ...
  setup(props, { emit }) {
    ...
    function loadMap() {
          ...
          navigator.geolocation.getCurrentPosition(function (position) {
            ...
            searchDetailAddrFromCoords(locPosition, function (result, status) {
              if (status === kakao.maps.services.Status.OK) {
                ...
                const message =
                  '<div style="width:100%;padding:5px;cursor:pointer" id="infowindow" >이 위치를 추가' +
                  '<div style="display:hidden" data-addr="' +
                  address +
                  '"></div>' +
                  "</div>"; // 인포윈도우에 표시될 내용
      ...
      function getListItem(index, places) {
        ...
        if (places.road_address_name) {
          itemStr +=
            ...
            "</span>" +
            '   <span class="jibun gray addr">' +
            places.address_name +
            "</span>";
        } else {
          itemStr +=
            '    <span class="addr">' + places.address_name + "</span>";
        }

        itemStr +=
          '  <span class="tel">' + places.phone + "</span>" + "</div>" + "<hr>";
      ...
      function displayInfowindow(marker, title) {
        var content =
          '<div style="width:100%;padding:5px;cursor:pointer">이 위치를 추가<div style="padding:5px;z-index:1;" data-addr="' +
          title +
          '">' +
          title +
          "</div></div>";
      ...
      function addinfoWindowClickEvt() {
        document
          ...
          .parentNode.parentNode.addEventListener("click", (e) => {
            addr.value =
              e.currentTarget.lastChild.childNodes[0].lastChild.getAttribute(
                "data-addr"
              );
